...
class QCNetAgentEncoder(nn.Module):
    ...
    def forward(self,
        ...
        mask = data['agent']['valid_mask'][:, time_shift:self.num_historical_steps+time_shift].contiguous()
        pos_a = data['agent']['position'][:, time_shift:self.num_historical_steps+time_shift, :self.input_dim].contiguous()
        motion_vector_a = torch.cat([pos_a.new_zeros(data['agent']['num_nodes'], 1, self.input_dim),
                                     pos_a[:, 1:] - pos_a[:, :-1]], dim=1)
        head_a = data['agent']['heading'][:, time_shift:self.num_historical_steps+time_shift].contiguous()
        ...
        if self.dataset == 'argoverse_v2':
            vel = data['agent']['velocity'][:, time_shift:self.num_historical_steps+time_shift, :self.input_dim].contiguous()
