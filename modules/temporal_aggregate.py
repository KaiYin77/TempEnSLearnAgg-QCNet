...
class TemporalAggregate(nn.Module):
    ...
    def forward(self,
                ...
                time_shift: int) -> Dict[str, torch.Tensor]:
        pos_m = data['agent']['position'][:, self.num_historical_steps - 1 + time_shift, :self.input_dim]
        head_m = data['agent']['heading'][:, self.num_historical_steps - 1 + time_shift]
        ...
        mask_src = data['agent']['valid_mask'][:, time_shift:self.num_historical_steps+time_shift].contiguous()
        ...
        pos_t = data['agent']['position'][:, time_shift:self.num_historical_steps+time_shift, :self.input_dim].reshape(-1, self.input_dim)
        head_t = data['agent']['heading'][:, time_shift:self.num_historical_steps+time_shift].reshape(-1)
        ...
        edge_index_pl2m_clone = edge_index_pl2m.clone()
        ...
        #### Temporal Aggregate Layer Start ####
        temporal_m = torch.cat([m.reshape(-1, 1, self.num_modes, self.hidden_dim), temporal_m], dim=1)
        temporal_m = torch.sum(temporal_m, dim=1)
        m = temporal_m.reshape(-1, self.hidden_dim)
        #### Temporal Aggregate Layer End ####
        for i in range(self.num_layers):
        ...
        return {
            ...
            'pi': pi,
            'final_m': m,
            'edge_index_pl2m_clone': edge_index_pl2m_clone
        }
