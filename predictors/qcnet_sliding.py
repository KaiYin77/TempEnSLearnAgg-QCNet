...
from utils import nms, k_means, weighted_k_means
...
class QCNetSliding(pl.LightningModule):
    ...
    def __init__(self,
                 ...
                 **kwargs) -> None:
        super(QCNetSliding, self).__init__()
        ...
        self.min_ade = [[] for _ in range(10)]
        self.min_fde = [[] for _ in range(10)]
        self.miss_rate = [[] for _ in range(10)]
        self.miss_count_by_nms = [[] for _ in range(10)]

    def forward(self, data: HeteroData, time_shift: int):
        scene_enc = self.encoder(data, time_shift)
        pred = self.decoder(data, scene_enc, time_shift)
        return pred, scene_enc

    def postprocess(self, data, traj_refine, pi, time_shift):
        ...
        batch_pi_eval = F.softmax(pi[eval_mask], dim=-1)
        return batch_sliced_traj_eval, batch_pi_eval
    
        
    def predict(self, batch_idx, data, time_shift):
        if isinstance(data, Batch):
            data['agent']['av_index'] += data['agent']['ptr'][:-1]
        reg_mask = data['agent']['predict_mask'][:, 50:]
        cls_mask = data['agent']['predict_mask'][:, -1]
        pred, _ = self(data, time_shift)
    ...
    def validation_step(self,
        ...
        for time_shift in range(10, 0, -1):
            # pred_traj is in current-agent-centric
            traj_refine, pi = self.predict(batch_idx, data, time_shift)
            # postprocess -> tranform from agent to world
            batch_sliced_traj_eval, batch_pi_eval = self.postprocess(data, traj_refine, pi, time_shift)
        ...
        eval_mask = data['agent']['category'] == 3
        batch_gt_sliced = data['agent']['position'][eval_mask, 60:, :2].detach().cpu().numpy()
        for idx in range(batch_gt_sliced.shape[0]):
            single_frame = batch_sliding_window_trajs[idx][:1].reshape(-1, 50, 2).detach().cpu().numpy().copy()
            s_fde_k, s_ade_k, s_miss = calculate_metrics(single_frame, batch_gt_sliced[idx])
            for window_size in range(1, 11):
                temp_sliced_window = batch_sliding_window_trajs[idx][:window_size].reshape(-1, 50, 2).detach().cpu().numpy().copy()
                clsfs = batch_sliding_window_clsfs[idx][:window_size].reshape(-1).detach().cpu().numpy()

                #k_means
                #kmeans_trajs, kmeans_clsfs = k_means(torch.from_numpy(temp_sliced_window), torch.from_numpy(clsfs))
                #fde, ade, miss = calculate_metrics(kmeans_trajs, batch_gt_sliced[idx])
                #ranking
                #sorted_indices = np.argsort(clsfs)[::-1]
                #top6_indices = sorted_indices[:6]
                #top6_trajs = temp_sliced_window[top6_indices]
                #fde, ade, miss = calculate_metrics(top6_trajs, batch_gt_sliced[idx])
                #nms
                nms_trajs, nms_clsfs = nms(temp_sliced_window, clsfs)
                fde, ade, miss = calculate_metrics(nms_trajs, batch_gt_sliced[idx])

                self.min_fde[window_size-1].append(fde)
                self.min_ade[window_size-1].append(ade)
                self.miss_rate[window_size-1].append(miss)
                if not s_miss and miss:
                    self.miss_count_by_nms[window_size-1].append(1)

    def on_validation_epoch_end(self):
        for window_size in range(1, 11):
            print(f'-- metric of 6*{window_size} according to best metric---')
            min_fde_avg = sum(self.min_fde[window_size - 1]) / len(self.min_fde[window_size - 1]) if len(self.min_fde[window_size - 1]) > 0 else 0
            min_ade_avg = sum(self.min_ade[window_size - 1]) / len(self.min_ade[window_size - 1]) if len(self.min_ade[window_size - 1]) > 0 else 0
            miss_rate_avg = sum(self.miss_rate[window_size - 1]) / len(self.miss_rate[window_size - 1]) if len(self.miss_rate[window_size - 1]) > 0 else 0
            miss_count_by_nms = sum(self.miss_count_by_nms[window_size - 1]) 
            
            print(f"Mean of min_ade_6-{window_size}: {min_ade_avg}")
            print(f"Mean of min_fde_6-{window_size}: {min_fde_avg}")
            print(f"Mean of miss_rate_6-{window_size}: {miss_rate_avg}")
            print(f"Mean of miss_count_by_nms-{window_size}: {miss_count_by_nms}")
