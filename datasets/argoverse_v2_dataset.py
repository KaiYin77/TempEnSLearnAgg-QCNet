...
class ArgoverseV2Dataset(Dataset):
    ...
    def __init__(self,
        ...
        if not os.path.isdir(root):
            os.makedirs(root)
        if split not in ('train', 'val', 'test', 'mini', 'densest', 'paper', 'video', 'thesis', 'slide-1', 'slide-2', 'ped', 'motor', 'tiny', 'viz_reflect'):
        ...
        if processed_dir is None:
            processed_dir = os.path.join(root, split, 'processed_qcnet')
    ...
    def _download(self) -> None:
        ...
        print(self.raw_dir, len(self.raw_file_names), len(self))
